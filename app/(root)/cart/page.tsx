      ...
      {items.length === 0 ? (
        ...
        <div className="md:w-4/5 w-[95%] mx-auto grid grid-cols-1 xl:grid-cols-6 gap-12">
          <div className="rounded-lg shadow-md overflow-hidden xl:col-span-4">
            ...
            {items.map((item) => (
              <div key={item.id} className="flex pb-6 mt-2 p-5 border-b-[1.5px] border-opacity-25 border-gray-700 items-center space-x-10">
                <div>
                ...
                <div>
                  ...
                  <h1 className="md:text-lg text-sm font-semibold text-black">Quantity: {item.quantity}</h1>
                  <h1 className="md:text-2xl text-base font-bold text-blue-950">${item.price.toFixed(2)}</h1>
                  <div className="flex items-center mt-4 space-x-2">
                    <Button onClick={() => dispatch(addItem(item))}>Add More</Button>
                    <Button variant={"destructive"} onClick={() => dispatch(removeItem({
  id: item.id,
  title: item.title,
  price: item.price,
  category: item.category,
  ...
  Remove
          ...
          <div className="xl:col-span-2">
            <div className="bg-indigo-950 sticky top-[25vh] p-6 rounded-lg">
              ...
              {!user ? (
                <Link href="/sign-in">
                  <Button className="bg-orange-600 w-full">Sign In to Checkout</Button>
                </Link>
              ) : (<div>
                <CheckoutForm
                ...
                />
                {/* <Link href="/checkout">
                <Button className="bg-orange-600 w-full">Buy Now</Button></Link>
                  */}
               </div>
              )}
            </div>
